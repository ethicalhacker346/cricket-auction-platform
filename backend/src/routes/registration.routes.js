...
router.get('/players', optionalAuthenticate, asyncHandler(registrationController.listPlayers));
...
router.patch(
  '/players/:registrationId/base-price',
  authenticate,
  authorize(USER_ROLES.ORGANIZER, USER_ROLES.ADMIN),
  validate(registrationIdParamSchema, 'params'),
  validate(setBasePriceSchema),
  asyncHandler(registrationController.setPlayerBasePrice)
);
...
router.get('/teams', optionalAuthenticate, asyncHandler(registrationController.listTeams));
